...
def get_split_points(dataloader,**kwargs):
...
class CustomDataSetScore(Dataset):
    ...
    def __init__(self, dataset, transform=[]):
        ...
        with torch.no_grad():
            self.average = torch.zeros((len(self.dataset)), dtype=torch.float)
            self.elapsed_epoch = torch.zeros((len(self.dataset)), dtype=torch.float)
            self.Scoredata = torch.ones((len(self.dataset)), dtype=torch.float)
    ...
    def contribute_average(self, ind, scores):
        ind = ind.clone().cpu()
        ...
        #print (self.elapsed_epoch.shape)
        
        with torch.no_grad():
            self.average[ind] = ((self.elapsed_epoch[ind]*self.average[ind]) + scores) / (self.elapsed_epoch[ind] + 1.0)
            self.elapsed_epoch[ind] += 1.0

    def transfer_ave(self):
        with torch.no_grad():
            self.Scoredata = self.average.clone()


    def setscores(self, ind, scores):
        #ind should be a tensor
        with torch.no_grad():
            self.Scoredata[ind] = scores

    def dropscores(self, mult):
        with torch.no_grad():
            self.Scoredata *= mult
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            return self.transform(x), y, self.Scoredata[index], index
        else:
            return x, y, self.Scoredata[index], index
...
def Median_Filter(tens, kernel):  #kernel=2, stride=1, quant=0):
